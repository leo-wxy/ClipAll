...
当前源码版本由根目录 [`VERSION`](../VERSION) 管理（`0.0.6`）。修改版本时必须同步 `Support/ClipAll-Info.plist` 的 `CFBundleShortVersionString` 和 `CFBundleVersion`，然后运行：
...
7. 验证插件停用、重新启用、保留配置卸载和删除配置卸载。
8. 开启开发者模式，加载未打包插件，验证重新载入、当前配置、单次执行、日志和 15 个 fixtures。
