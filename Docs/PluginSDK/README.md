...
ClipAll 外置插件是后缀为 `.clipallplugin` 的目录 package。v2 面向“根据选中文字产生结构化结果”的本地转换能力：插件声明匹配规则、配置和 handler，宿主负责推荐、配置界面、结果 UI、复制与生命周期。
...
var ClipAllPlugin = {
  run: function (text) {
    var configuration = App.getPluginEnv("com.example.my-plugin");
    return {
      title: "结果",
      subtitle: null,
      items: [
        { id: "value", label: "值", value: text, style: "body" }
      ]
    };
  }
};
...
完整字段见 [manifest-v2.md](manifest-v2.md)，执行环境见 [runtime-v2.md](runtime-v2.md)，开发与排错见 [debugging.md](debugging.md)。参考实现位于 `Plugins/Examples/TimestampTools.clipallplugin`。
...
5. 卸载：删除宿主管理的安装副本、普通配置与 Keychain 密钥；源包不变。停用或升级会保留配置。
...
它支持“跟随系统 / UTC”时区和“标准 / ISO 8601 / 中文”显示格式。卸载会清除安装副本与配置，但不会删除 App 内的示例资源或用户选择的源目录。
...
- v2 不支持外置插件 secret、自定义 UI、外部 URL 或后台任务。
...
- handler 必须是纯函数式行为：相同文本与配置应产生相同结果；本地时区行为使用标准 JavaScript 环境。
