            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                if !lifecycle.invalidPlugins.isEmpty {
                    VStack(alignment: .leading, spacing: 5) {
                        Label(
                            "\(lifecycle.invalidPlugins.count) 个插件载入失败",
                            systemImage: "exclamationmark.triangle.fill"
                        )
                        .font(.caption.weight(.semibold))
                        .foregroundStyle(.orange)
                        ForEach(lifecycle.invalidPlugins.prefix(2)) { invalid in
                            Text("\(invalid.packageURL.lastPathComponent)：\(invalid.issue.errorDescription ?? invalid.issue.message)")
                                .font(.caption2)
                                .foregroundStyle(.secondary)
                                .lineLimit(2)
