...
- [x] 实现指针选择手势状态机（拖动或双/三击）、防抖、普通单击抑制、重复选区抑制及 Carbon 全局快捷键入口。
...
| TextEdit 拖动选择 | 浮层靠近选区，复制/搜索/翻译可用 |
| 双击/三击选词 | 完成单词或段落选择后自动弹出浮层 |
| 普通单击旧高亮 | 不读取残留选区，不重新弹出浮层 |
| Safari 可访问文本 | 能捕获时正常显示；目标不支持时安全退出 |
| 键盘选择 + 快捷键 | 使用与指针相同的浮层、路由和执行链路 |
| 普通键盘输入 | 不监听或重放普通按键，来源应用只输出一次字符 |
